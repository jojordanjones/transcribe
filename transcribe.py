...
def setup_openai() -> None:
    """Load API key from .env and initialize openai."""
    load_dotenv()
...
def transcribe_audio_file(path: Path, model: str = "whisper-1", language: str | None = None) -> str:
    """Transcribe the entire audio file and return the combined text."""
    audio = AudioSegment.from_file(path)
    chunks = split_audio(audio)
    texts = [
        transcribe_segment(chunk, idx, path, model=model, language=language)
...
def main() -> None:
    ...
    result = transcribe_audio_file(args.input, model=args.model, language=args.language)
    if args.output:
